...
def index(request):
    ...
    categories_list = Category.objects.filter(products__type__name='video').distinct()
    ...
    return render(request, 'videos_select.html', {'logged': logged, 'user_name': user_name, 'categories': categories, 'categories_list': categories_list})
...
def search(request):
    ...
    try:
        type_obj = Type.objects.get(name='video')
    except Type.DoesNotExist:
        raise Http404('Stránka neexistuje!')
